#!/usr/bin/env python3
"""Path36: independent physical all-slice knee-MRI specialist.

One small from-scratch family: fixed-mm public preprocessing, patient-coordinate
laterality normalization, separate plane encoders, and target-routed slice pooling.
Gold images enter only fold-clean adaptation and the final full-gold deployment fit.
Report-label policy selection is five-fold cross-fitted on the immutable gold folds.
"""
...
def build_cache(volumes: TwoPartArray, masks: np.ndarray, ids: np.ndarray,
                raw_root: Path) -> tuple[Path, list[str]]:
    final = Path(f"/kaggle/working/path36_physical{IMAGE_SIZE}.npy")
    sides, scanners = [], []
    tmp = final.with_suffix(".tmp.npy")
    cache = np.lib.format.open_memmap(tmp, mode="w+", dtype=np.uint8,
                                      shape=(len(ids), 44, IMAGE_SIZE, IMAGE_SIZE))
    counts = Counter()
    started = time.time()
    for index, study in enumerate(ids):
        ...
        resized = torch.nn.functional.interpolate(
            torch.from_numpy(np.array(volume, copy=True)).float().unsqueeze(0),
            size=(IMAGE_SIZE, IMAGE_SIZE), mode="area",
        ).round().clamp(0, 255).to(torch.uint8).squeeze(0).numpy()
        cache[index] = resized
        masks[index] = resized.reshape(44, -1).any(axis=1)
        if (index + 1) % 500 == 0:
            log(f"cache {index + 1}/{len(ids)} ({(time.time() - started) / 60:.1f} min)")
    cache.flush()
    del cache
    os.replace(tmp, final)
    log(f"cache complete: {final.stat().st_size / 1024**3:.2f} GiB; laterality={dict(counts)}")
    return final, scanners
...
class VolumeDataset(Dataset):
    ...
    def __getitem__(self, item: int):
        ...
        x = torch.from_numpy(np.array(self.cache[row], copy=True))
        if self.augment:
            gain = 0.9 + 0.2 * random.random()
            gamma = 0.9 + 0.2 * random.random()
            x = (255 * ((x.float() / 255).pow(gamma) * gain).clamp(0, 1)).to(torch.uint8)
...
class SliceEncoder(nn.Module):
    def __init__(self, feature_dim: int = 128):
        super().__init__()
        channels = (1, 16, 32, 64, feature_dim)
        layers = []
        for before, after in zip(channels[:-1], channels[1:]):
            layers += [
                nn.Conv2d(before, after, 3, padding=1, bias=False),
                nn.InstanceNorm2d(after, affine=True),
                nn.SiLU(inplace=True),
                nn.Conv2d(after, after, 3, padding=1, groups=after, bias=False),
                nn.InstanceNorm2d(after, affine=True),
                nn.SiLU(inplace=True),
                nn.AvgPool2d(2),
            ]
...
class PhysicalSpecialist(nn.Module):
    def __init__(self, feature_dim: int = 128):
        super().__init__()
        self.encoders = nn.ModuleList([SliceEncoder(feature_dim) for _ in range(3)])
    ...
    def forward(self, volume: torch.Tensor, present: torch.Tensor) -> torch.Tensor:
        x = volume.float().div_(255).sub_(0.5).div_(0.25)
        features = []
        for encoder, (start, stop) in zip(self.encoders, PLANE_BOUNDS):
            plane = x[:, start:stop].flatten(0, 1).unsqueeze(1)
            features.append(encoder(plane).view(len(x), stop - start, -1))
        features = torch.cat(features, dim=1) + self.slot_embedding[self.slot_ids]
...
def fit_model(cache_path: Path, masks: np.ndarray, labels: np.ndarray, weights: np.ndarray,
              ...
              learning_rate: float = 2e-3) -> tuple[PhysicalSpecialist, list[dict], int]:
    seed_everything(seed)
    workers = 2 if device.type == "cuda" else 0
    train_ds = VolumeDataset(cache_path, masks, labels, weights, train_indices, True)
    ...
    val_loader = None if val_indices is None else DataLoader(
        VolumeDataset(cache_path, masks, labels, weights, val_indices, False),
        batch_size=BATCH_SIZE, shuffle=False, num_workers=workers,
        pin_memory=device.type == "cuda", persistent_workers=workers > 0,
    )
    model = PhysicalSpecialist().to(device)
    ...
    for epoch in range(epochs):
        model.train()
        total = 0.0
        for volume, present, truth, confidence in train_loader:
            volume, present = volume.to(device, non_blocking=True), present.to(device, non_blocking=True)
            truth, confidence = truth.to(device, non_blocking=True), confidence.to(device, non_blocking=True)
            optimizer.zero_grad(set_to_none=True)
            with torch.amp.autocast("cuda", dtype=torch.float16, enabled=device.type == "cuda"):
                logits = model(volume, present)
...
def self_test() -> None:
    ...
    model = PhysicalSpecialist(feature_dim=16)
    volume = torch.randint(0, 256, (2, 44, 32, 32), dtype=torch.uint8)
    present = torch.ones(2, 44, dtype=torch.uint8)
    present[1, 12:22] = 0
    logits = model(volume, present)
    assert logits.shape == (2, 12) and torch.isfinite(logits).all()
    logits.sum().backward()
    print("PATH36_SELF_TEST_OK")
...
def main() -> None:
    ...
    if device.type != "cuda":
        raise RuntimeError("Path36 training requires a GPU")
    ...
    cache_path, scanner_list = build_cache(volumes, masks, ids, raw_root)
    ...
    for fold in range(5):
        ...
        held_loader = DataLoader(
            VolumeDataset(cache_path, masks, fold_labels, fold_weights, held_gold, False),
    ...
    checkpoint = {
        ...
        "architecture": "three-plane 256px all-slice CNN with target-routed attention-plus-max pooling",
        "targets": TARGETS,
    ...
    torch.save(checkpoint, "/kaggle/working/path36_physical_specialist.pt")
    ...
    prediction_frame.to_csv("/kaggle/working/path36_gold_predictions.csv", index=False)
    audit = {
        "status": "PATH36_PHYSICAL_SPECIALIST_TRAIN_COMPLETE",
        "hypothesis": "full-corpus competition-aligned labels plus physical all-slice target specialists add independent rank signal",
        "independence": "no public prediction, DINO checkpoint, Raptor weight, or Claude path used",
        "architecture_selection_note": (
            "256px matches the published ELNet knee-MRI regime; the prior 128px run's full-gold "
            "failure was observed, so this is fold-clean training evidence but not a never-seen architecture test"
        ),
        "physical_contract": {
            "source": "CC0 fixed-140mm five-slot corpus",
            "slice_processing": "individual all-slice encoding; no cross-slot 2.5D windows",
            "laterality": "right knees canonicalized from DICOM patient coordinates/tags",
            "plane_encoders": ["sagittal", "coronal", "axial"],
            "slot_lengths": SLOT_LENGTHS,
            "image_size": IMAGE_SIZE,
        },
    ...
    Path("/kaggle/working/path36_audit.json").write_text(json.dumps(audit, indent=2) + "\n")
    cache_path.unlink()
    log(f"Path36 crossfit gold macro AUC={gold_macro:.5f}; per-target={gold_per_target}")
    log("PATH36_PHYSICAL_SPECIALIST_TRAIN_COMPLETE")
