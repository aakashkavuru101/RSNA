"""Path32 — member v3 single-fold probe: bag-sampled windows, 150mm @224px.
...
T0 = time.time()
SEED = 20260903  # same seed/fold as v3f0: isolates the epoch effect
SMOKE = False
SMOKE_NON_GOLD = 768
EPOCHS = 2 if SMOKE else 20
IMG_SIZE = 224
CROP_MM = 150.0
SLICE_BAND = (0.08, 0.92)
GROUP_SIZE = 3
N_GROUPS = 1 if SMOKE else 4
...
HEADER_THREADS = 16
DECODE_THREADS = 10
TIME_LIMIT_S = 3.2 * 3600
FOLD_START_DEADLINE_S = 0.0  # single-fold run: fold 0 only   # do not begin a new fold after this point

# --- Member config ----------------------------------------------------------
MODEL_KEY = "base"
MEMBER_TAG = "dino2b_v3e20"
...
def take_window(cache_rows: np.ndarray, start: int) -> np.ndarray:
    """Contiguous 3-slice window at an arbitrary start (v3 bag-sampling)."""
...
N_WINDOW_STARTS = None  # set after CACHE_SLICES known


def eval_starts() -> list[int]:
    last = CACHE_SLICES - GROUP_SIZE
    return sorted(set([0, last // 3, 2 * last // 3, last]))
...
def predict(model, cache, mask, indices, device) -> np.ndarray:
    ...
    model.eval()
    group_outputs = []
    for w_start in eval_starts():
        outputs = []
        for start in range(0, len(indices), EVAL_BATCH):
            selected = indices[start:start + EVAL_BATCH]
            images = torch.from_numpy(take_window(cache[selected], w_start)).to(
...
def train_model(model, cache, mask, table, train_indices, val_indices, gold_indices, device):
    ...
    for epoch in range(EPOCHS):
        ...
        for start in range(0, len(permutation) - BATCH_STUDIES + 1, BATCH_STUDIES):
            ...
            w_start = int(np.random.randint(0, CACHE_SLICES - GROUP_SIZE + 1))
            images = torch.from_numpy(take_window(cache[selected], w_start)).to(
                device, non_blocking=True
            )
...
def main() -> None:
    ...
    run_folds = 1  # v3 probe: scanner fold 0 only
    table = labels.set_index("StudyInstanceUID").loc[studies]
    targets = table[TARGETS].values.astype(np.float32)
    gold_columns = [f"{target}__gold" for target in TARGETS]
    gold_truth = table[gold_columns].values.astype(np.float32)
    ...
    for fold in range(run_folds):
        ...
        if False and available_folds != 5:
    ...
    partial = True  # single-fold probe: OOF covers fold 0 only
    eligible = np.flatnonzero(~table["is_gold"].values)
