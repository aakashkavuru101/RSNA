"""Ensemble inference vehicle for RSNA Knee MRI.

Loads every member checkpoint package (each = 5 folds of one backbone config)
found under /kaggle/input, decodes the TEST set once, and averages raw
probabilities across members (TTA included) before the single final rank
transform. Members are discovered by checkpoint filename pattern
``<tag>_fold[0-4].pt``; each checkpoint carries the preprocessing contract
(img_size, crop_mm, slice_band, group_size, n_groups, cache_slices, targets,
slots) and model_key that gate the load.

Submission candidates are written as rank-mean over members. Integrity gates
hard-fail; a member whose package is incomplete or whose contract mismatches
is skipped and recorded loudly in the audit (never silently blended).
"""
...
def main() -> None:
    ...
    for tag, folds in sorted(members.items()):
        try:
            ...
            log(f"member '{tag}': {N_FOLDS} folds averaged (TTA={TTA})")
        except Exception as exc:
    ...
    audit = {
        "mode": "ensemble-member-inference",
        ...
        "status": "ENSEMBLE_SUBMISSION_WRITTEN",
    }
    Path("ensemble_audit.json").write_text(json.dumps(audit, indent=2, default=float))
    log("ensemble_audit.json:\n" + json.dumps(audit, indent=2))
