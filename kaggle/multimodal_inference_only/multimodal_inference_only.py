"""Multimodal-only ensemble inference kernel for RSNA Knee MRI.

Loads the dino2b_mm6 five-fold checkpoint package (DINOv2-Base trained with
InfoNCE report-text alignment on silver_labels_v6 + full-gold), decodes the
TEST set once, and rank-mean averages per-fold predictions (with label-aware
horizontal-flip TTA) into submission.csv.

The multimodal architecture adds text_proj + clip_temp on top of the standard
KneeDINO. state_dict loading tolerates missing/extra keys.

Integrity gates hard-fail; missing or contract-mismatched checkpoints are
skipped and recorded loudly in the audit (never silently blended).
"""
...
def main() -> None:
    ...
    for tag, folds in sorted(members.items()):
        if ONLY_MULTIMODAL and tag != MEMBER_TAG:
            log(f"SKIP member '{tag}': only multimodal allowed, this is not {MEMBER_TAG}")
            skipped[tag] = "not multimodal"
            continue
        try:
            fold_preds = []
            arch = None
