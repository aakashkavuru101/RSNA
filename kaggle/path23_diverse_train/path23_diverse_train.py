"""Path23 — gold-free CNN-backbone training for RSNA Knee MRI ensemble diversity.

Every production model so far is a ViT (DINOv2/DINOv3). This kernel retrains the
proven stage-0 pipeline (clean_dino_train / path20 stage 0) on a convolutional
backbone, EfficientNet-B3, so the ensemble gains an architecturally independent
member. The stage-0 machinery is reused verbatim: DICOM decode, slot/laterality
pipeline, build_cache, scanner-grouped 5-fold split, train loop, honest gold
monitor, test inference, write_submission, audit.

GOLD-FREE CONTRACT: no gold override, no gold stages, gold_training_count=0 in
every checkpoint and the audit. The 58 gold studies are used only as a
read-only monitor, exactly as stage-0 of the template does (never trained on,
never used for checkpoint selection).

Backbone weight source (internet is OFF during the run): the official Kaggle
model mirror "timm/tf-efficientnet/PyTorch/tf-efficientnet-b3/1" (declared in
kernel-metadata model_sources, the same source the mirrored public kernel
prvsiyan/rsna-knee-read-the-report-then-the-knee declares), discovered by a
/kaggle/input walk. The public B3 line's hash-pinned copy
(timm-pretrained-weights/efficientnet_b3.pth, sha256 pinned below) is also
accepted if attached as a dataset. Loading follows the public precedent:
timm.create_model(..., pretrained=False) + strict load_state_dict with shape
checks; the pinned sha256 is a recorded audit field, strict state-dict load is
the hard contract.

Preprocessing deviations from the template: none in the data contract. The
only structural change is dropping the 224 px baseline cache (it existed only
to feed the parent-kernel blend guard, which this kernel does not have);
IMG_SIZE stays 336 so the slot cache is byte-identical to the DINO line.
"""
...
IMG_SIZE = 336  # same as the DINO line; the B3 CNN accepts any resolution
CROP_MM = 130.0
...
BATCH_STUDIES = 3
EVAL_BATCH = 4
UNFREEZE_LAST = 2  # EfficientNet-B3 block groups (plus conv_head/bn2)
LR_BACKBONE = 1.0e-5
LR_HEAD = 8.0e-4
WEIGHT_DECAY = 0.02
HEADER_THREADS = 16
DECODE_THREADS = 10
# Path23: single T4, stage budget <= ~5.5h (decode + 5 folds + inference).
TIME_LIMIT_S = 5.5 * 3600
# Soft stop: finish the current fold, then write a partial audit + submission
# from the completed folds instead of dying silently.
PATH23_FOLD_GUARD_S = 0.5 * 3600

# --- Path23: diverse CNN backbone, gold-free ---
# Single top-level pointer to the silver labels CSV; this run uses
# silver_labels_mv1 (dataset aakashkavuru/rsna-knee-silver-labels-mv1). A rerun
# on a newer label set is a one-line change here (plus the labels dataset in
# kernel-metadata.json).
LABELS_FILE = "silver_labels_mv1.csv"
BACKBONE_NAME = "efficientnet_b3"
# sha256 of the public B3 line's hash-pinned copy
# (/kaggle/input/timm-pretrained-weights/efficientnet_b3.pth in
# prvsiyan/rsna-knee-read-the-report-then-the-knee). Recorded in the audit;
# if the attached mirror differs, the strict load_state_dict remains the
# hard integrity contract.
B3_WEIGHTS_PINNED_SHA256 = "b2fd73dffb7386bf35ed9f571e98c1ab53647ed435268eb629c307ad221b5b31"
...
def find_b3_weights() -> Path:
    """Locate the offline EfficientNet-B3 ImageNet weights.

    Primary source: the Kaggle model mirror
    "timm/tf-efficientnet/PyTorch/tf-efficientnet-b3/1" (mount path contains
    "efficientnet-b3"). Fallback: the public line's dataset copy
    "timm-pretrained-weights/efficientnet_b3.pth". If several files match, the
    one whose sha256 equals the public pin wins; otherwise exactly one unique
    candidate is required.
    """
    weight_ext = (".pth", ".pt", ".bin")
    name_pattern = re.compile(r"efficientnet.*b3.*\.(pth|pt|bin)$", re.IGNORECASE)
    candidates = []
    for root, directories, files in os.walk("/kaggle/input"):
        directories[:] = [d for d in directories if d not in ("train_series", "test_series")]
        text = str(root).lower()
        dir_match = "efficientnet" in text and "b3" in text
        for filename in files:
            lower = filename.lower()
            if "fold" in lower:
                continue  # never pick up our own or public fold checkpoints
            if dir_match and lower.endswith(weight_ext):
                candidates.append(Path(root) / filename)
            elif name_pattern.fullmatch(lower):
                candidates.append(Path(root) / filename)
    unique = sorted(set(candidates), key=lambda p: len(str(p)))
    if not unique:
        raise FileNotFoundError(
            "EfficientNet-B3 weights not found under /kaggle/input; attach "
            "model source timm/tf-efficientnet/PyTorch/tf-efficientnet-b3/1"
        )
    if len(unique) > 1:
        pinned = [p for p in unique if sha256_file(p) == B3_WEIGHTS_PINNED_SHA256]
        if len(pinned) == 1:
            return pinned[0]
        raise FileNotFoundError(
            f"Ambiguous EfficientNet-B3 weight files: {[str(p) for p in unique]}"
        )
    return unique[0]
...
def validate_and_join_labels(train_df: pd.DataFrame, label_path: Path) -> pd.DataFrame:
    ...
    log(f"labels: {len(merged)} studies, {n_gold} gold monitor-only, source={label_path.name}")
    return merged
...
class KneeCNN(nn.Module):
    """EfficientNet-B3 slice encoder + the template's study-level SlotHead.

    Mirrors KneeDINO's feature contract: each slot is one GROUP_SIZE-channel
    image (interleaved central slices), global pooled to a 2 * num_features
    vector (mean + max, standing in for [cls | mean-patch]) before SlotHead.
    """

    def __init__(self, backbone: nn.Module):
        super().__init__()
        self.backbone = backbone
        self.head = SlotHead(backbone.num_features * 2, len(SLOTS), len(TARGETS))
    ...
    def forward(self, images: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        fmap = self.backbone(pixels)  # global_pool="" -> (N, C, h, w)
        features = torch.cat([fmap.mean(dim=(2, 3)), fmap.amax(dim=(2, 3))], dim=1)
        return self.head(features.reshape(batch, slots, -1), mask)


def build_model(weights_path: Path) -> KneeCNN:
    import timm

    backbone = timm.create_model(
        BACKBONE_NAME, pretrained=False, in_chans=GROUP_SIZE,
        num_classes=0, global_pool="",
    )
    # Offline weight load, public-line precedent: strict state-dict match with
    # per-tensor shape checks; no network access anywhere in this path.
    ...
    if "conv_stem.weight" in state and state["conv_stem.weight"].shape[1] != GROUP_SIZE:
        if state["conv_stem.weight"].shape[1] == 1:
            state["conv_stem.weight"] = state["conv_stem.weight"].repeat(1, GROUP_SIZE, 1, 1)
        else:
            raise RuntimeError(
                f"conv_stem.weight has {state['conv_stem.weight'].shape[1]} input "
                f"channels, expected {GROUP_SIZE}"
            )
    ...
    log(
        f"B3 weights: {weights_path} sha256={weights_sha256[:16]}... "
        f"(public pin {'MATCH' if weights_sha256 == B3_WEIGHTS_PINNED_SHA256 else 'DIFFERS'})"
    )
    ...
    for module in (backbone.conv_head, backbone.bn2):
        for parameter in module.parameters():
            parameter.requires_grad = True
    trainable = sum(p.numel() for p in backbone.parameters() if p.requires_grad)
    log(f"B3: {len(blocks)} block groups, last {UNFREEZE_LAST} open, {trainable/1e6:.1f}M trainable")
    return KneeCNN(backbone)
...
def affine_augment(images: torch.Tensor) -> torch.Tensor:
    # Rotation/scale/translate/gain only: no horizontal flip, so medial/lateral
    # labels never swap (laterality is already canonicalized at decode time).
...
def make_clean_split(
    ...
    log(
        ...
        f"gold={len(gold_indices)} (monitor only), "
...
def write_benchmark_submission(test_df, path="submission.csv"):
    """Write the 0.5 file immediately.

    A submission that never writes scores nothing at all. A kill for memory is
    a SIGKILL and never reaches an exception handler, so a valid file exists
    from the first minute and is overwritten only once real predictions land.
    """
    output = test_df[["StudyInstanceUID"]].copy()
    for target in TARGETS:
        output[target] = 0.5
    output.to_csv(path, index=False)
    log(f"wrote benchmark {path} (0.5 fill); will be overwritten by real predictions")
...
def main() -> None:
    ...
    # Contract guard: a valid submission.csv exists before any heavy work.
    ...
    path23_status = "PATH23_DIVERSE_TRAINED"
    partial_reason = None

    for fold in range(run_folds):
        if fold > 0 and time.time() - T0 > TIME_LIMIT_S - PATH23_FOLD_GUARD_S:
            path23_status = "PATH23_PARTIAL_TIMEOUT"
            partial_reason = f"time guard before fold {fold}"
            log(f"Path23: stopping before fold {fold} (time guard)")
            break
        ...
        try:
            ...
            log(f"Path23: fold {fold} interrupted by time guard: {exc}")
            break
        ...
        checkpoint_name = "path23_diverse_smoke.pt" if SMOKE else f"path23_diverse_fold{fold}.pt"
        torch.save(checkpoint, checkpoint_name)
        checkpoint_hashes[checkpoint_name] = sha256_file(Path(checkpoint_name))
        ...
        log(f"Path23 fold {fold}: saved {checkpoint_name}")
        del model, state
        gc.collect()
        torch.cuda.empty_cache()

    if not completed_folds:
        path23_status = "PATH23_FALLBACK_BENCHMARK"
        partial_reason = partial_reason or "no fold completed; benchmark submission retained"
        log("Path23: no fold completed; submission.csv remains the 0.5 benchmark")

    # Pooled scanner-isolated silver OOF across the completed folds. Gold is a
    # read-only monitor: never trained on, never used for checkpoint selection.
    covered = eligible[np.isfinite(new_oof[eligible]).all(axis=1)]
    oof_truth = (targets[covered] > 0.5).astype(int)
    oof_rank = rank_predictions(new_oof[covered]) if len(covered) else new_oof[covered]
    new_oof_auc, oof_per_target = macro_auc(oof_truth, oof_rank)
    log(f"pooled scanner-isolated OOF macro AUC ({len(covered)} studies): {new_oof_auc:.4f}")

    gold_columns = [f"{target}__gold" for target in TARGETS]
    gold_truth = table[gold_columns].values[gold_indices].astype(int)
    gold_auc = float("nan")
    gold_per_target = {}
    if fold_gold_predictions:
        gold_ensemble = rank_predictions(np.mean(fold_gold_predictions, axis=0))
        gold_auc, gold_per_target = macro_auc(gold_truth, gold_ensemble)
        log(f"gold monitor (read-only ensemble): {gold_auc:.4f}")

    if fold_test_predictions:
        test_ensemble = rank_predictions(np.mean(
    ...
    metrics = {
        ...
        "pooled_silver_oof_auc": new_oof_auc,
        "silver_oof_per_target": oof_per_target,
        "oof_covered_studies": int(len(covered)),
        ...
        "status": path23_status,
        "elapsed_seconds_before_submission": time.time() - T0,
    }
    Path("metrics.json").write_text(json.dumps(metrics, indent=2))

    # OOF export for downstream ensembling: silver rows are true
    # scanner-isolated OOF; gold rows are the 5-fold monitor ensemble
    # (gold never entered training or early stopping, so the read is honest).
    oof_frame = pd.DataFrame(new_oof, columns=TARGETS)
    ...
    for fold in completed_folds:
        ...
        fold_of[va_pos] = fold
    if fold_gold_predictions:
        oof_frame.loc[gold_indices, TARGETS] = np.mean(fold_gold_predictions, axis=0)
    oof_frame["fold"] = fold_of
    oof_frame["is_gold"] = table["is_gold"].values.astype(int)
    complete_rows = np.isfinite(oof_frame[TARGETS].to_numpy(np.float64)).all(axis=1)
    if not complete_rows.all():
        if not SMOKE and path23_status == "PATH23_DIVERSE_TRAINED":
            raise RuntimeError("Path23 OOF has uncovered studies")
        oof_frame = oof_frame.loc[complete_rows]
    oof_frame.to_csv("path23_diverse_oof.csv", index=False)

    audit = {
        "status": path23_status,
        "partial_reason": partial_reason,
        "mode": "path23-diverse-train",
        "parent_line": "clean_dino_train stage-0 machinery; no blend parents",
        "smoke": SMOKE,
        "backbone": BACKBONE_NAME,
        "backbone_weights_path": str(weights_path),
        "backbone_weights_sha256": weights_sha256,
        "backbone_weights_public_pin_sha256": B3_WEIGHTS_PINNED_SHA256,
        "backbone_weights_public_pin_match": bool(
            weights_sha256 == B3_WEIGHTS_PINNED_SHA256
        ),
        ...
        "gold_training_studies": 0,
        "gold_protocol": (
            "gold-free: the 58 gold studies are excluded from every training "
            "split and from checkpoint selection; they are read only as a "
            "monitor, exactly as stage-0 of the template"
        ),
        "pooled_silver_oof_macro_auc": float(new_oof_auc),
        "silver_oof_per_target_auc": oof_per_target,
        "oof_covered_studies": int(len(covered)),
        "gold_monitor_macro_auc": float(gold_auc),
        "gold_monitor_per_target": gold_per_target,
        "completed_folds": completed_folds,
        "checkpoints": checkpoint_hashes,
        "artifacts": dict(checkpoint_hashes),
        "elapsed_seconds": time.time() - T0,
    }
    if Path("path23_diverse_oof.csv").is_file():
        audit["artifacts"]["path23_diverse_oof.csv"] = sha256_file(
            Path("path23_diverse_oof.csv")
        )
    Path("path23_audit.json").write_text(json.dumps(audit, indent=2, default=float))
    log(f"Path23 complete: status={path23_status} in {(time.time() - T0) / 3600:.2f} hours")
