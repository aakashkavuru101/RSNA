"""Ensemble-member training kernel for RSNA Knee MRI.

One configurable backbone (DINOv2 small/base/large), 6 acquisition slots,
130 mm physical crop, 336 px, scanner-grouped 5-fold, FULL-GOLD training
(the 58 expert studies join every fold's training at per-cell weight lambda),
TTA (label-aware horizontal flip) at inference, and a submission.csv emitted
directly from the 5-fold test ensemble.

Deliberate differences from the clean_dino_train parent:
  * no 224 px baseline reproduction and no resolution blend (that was a
    one-time experiment; this kernel trains only the target backbone);
  * the 58 gold studies are training data (GOLD_INTEGRATION_PLAN.md §2:
    deployment at full gold is authorized after the recipe is cross-fitted;
    this kernel is the deployment leg);
  * backbone is selected by MODEL_KEY + model_sources in kernel metadata.

Config is at the top of the file. SEED is varied per member for diversity.
"""
...
# --- Member config ----------------------------------------------------------
MODEL_KEY = "base"          # small | base | large  (matches model_sources path)
MEMBER_TAG = "dino2b"       # prefix for checkpoint filenames
LABELS_FILE = "silver_labels_v5.csv"  # -> v6 once the new extractor corpus is fused
GOLD_LAMBDA = 2.0           # per-cell loss weight on the 58 gold rows
TTA = True                  # label-aware horizontal-flip TTA at inference
...
# Laterality-aware flip swaps: TTA hflip moves medial<->lateral content.
    ...
    if tta:
        ...
        for group in range(N_GROUPS):
            ...
            for start in range(0, len(indices), EVAL_BATCH):
                ...
                images = images.flip(dims=[-1])  # horizontal flip
                present = torch.from_numpy(mask[selected]).to(device, non_blocking=True)
                with torch.autocast("cuda", enabled=device.type == "cuda"):
                    logits = model(images, present)
                outputs.append(torch.sigmoid(logits).float().cpu().numpy())
...
def train_model(model, cache, mask, table, train_indices, val_indices, gold_indices, device):
    ...
    # Full-gold deployment: gold rows join training with binary targets at
    # weight GOLD_LAMBDA. Never evaluated (gold monitor is in-sample here).
...
def main() -> None:
    ...
    if not SMOKE and not np.isfinite(oof).all():
        raise RuntimeError("OOF predictions do not cover every study")
